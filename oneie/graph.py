class Graph(object):
    ...
    def to_dict(self):
        ...
        if 'relation_subtype' in self.vocabs:
            sub_relation_itos = {i: s for s, i in self.vocabs['relation_subtype'].items()}
        else:
            sub_relation_itos = {}
        ...
        if len(self.sub_relations) == len(self.relations):
            relations = [[i, j, relation_itos[k], l, sub_relation_itos[m]] for (i, j, k), l, m in zip(self.relations, self.relation_scores, self.sub_relations)]
        else:
    ...
    def clean(self, relation_directional=False, symmetric_relations=None):
        ...
        relations = [(i, j, k, l, m) for (i, j, k), l, m in zip(self.relations, self.relation_scores, self.sub_relations)]
        ...
        if relation_directional and symmetric_relations:
            relation_itos = {i: s for s, i in self.vocabs['relation_type'].items()}
            relations_tmp = []
            for i, j, k, l, m in relations:
                if relation_itos[k] not in symmetric_relations:
                    relations_tmp.append((i, j, k, l, m))
                else:
                    if j < i:
                        i, j = j, i
                    relations_tmp.append((i, j, k, l, m))
        ...
        self.relations = [(i, j, k) for i, j, k, _, _ in relations]
        self.relation_scores = [l for _, _, _, l, _ in relations]
        self.sub_relations = [m for _, _, _, _, m in relations]
